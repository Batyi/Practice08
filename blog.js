...
server.post(['/entry/:entryID/comment/create',
    '/entry/:entryID/comment/:id/update'], (request, response) => {
    if (!request.session.authorized) {
        response.status(401).end('Unauthorized');
        return;
    }
...
server.get(['/entry/:entryID/comment/:id/update'], (request, response) => {
	...
	id = request.params['id'];
	if (!entryID || !id) {
		request.session.errors.push('The blog comment is unknown.');
		response.redirect(previousLocation);
		return;
	}
	Entry.findById(entryID).then(entry => {
		Comment.findById(id).then((comment) => {
			...
			console.error(error);
			request.session.errors.push('Failed to find the specified blog comment.')
			response.redirect(previousLocation);
		});
	}).catch(error => {
		console.error(error);
		request.session.errors.push('Failed to find the specified blog entry before finding blog comment.')
		response.redirect(previousLocation);
	});
